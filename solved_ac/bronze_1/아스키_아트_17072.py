# 백준 - 브론즈1 - 아스키 아트 - 17072 - 단순 구현 문제
'''
단순 구현 문제

문제를 처음 봤을 땐 어려워보였는데, 읽어보면 단순 구현 문제이다.

풀이 과정
 - r, g, b 를 각각 곱하고 i라는 이름으로 더한다.
 - i 값의 범위에 따라 해당하는 문자열을 리턴한다.
    - 이때 r, g, b를 구해야 된다. 즉, len(i) - 2까지만 반복해야 된다.
 - 리턴한 값들을 모아서 temp에 담아 둔 뒤, res에 옮기고 res를 출력하면 된다.
'''
...
for i in board:
    ...
    for j in range(0, len(i) - 2, 3):
        temp.append(intensity(i[j], i[j + 1], i[j + 2]))
